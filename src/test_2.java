...
public class test_2 {
    ...
    static void createNewFile() {
        currentFile = directoryPath + File.separator + JOptionPane.showInputDialog("Enter new file name:") + ".txt";
    ...
    static void readFileByName() {
        String fileName = JOptionPane.showInputDialog("Enter the name of the file to read (without .txt extension):");
        if (fileName != null && !fileName.isEmpty()) {
            fileName = fileName + ".txt"; // Add .txt extension
            Path filePath = Paths.get(directoryPath, fileName);
            JOptionPane.showMessageDialog(null, "Looking for file: " + filePath.toAbsolutePath());

            if (Files.exists(filePath) && Files.isRegularFile(filePath)) {
                currentFile = filePath.toString();
                StringBuilder fileContent = new StringBuilder("File content:\n");
